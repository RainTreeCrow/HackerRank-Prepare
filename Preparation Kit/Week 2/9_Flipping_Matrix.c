...
int flippingMatrix(int matrix_rows, int matrix_columns, int** matrix) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            if (matrix[i][2 * n - 1 - j] > max_value) {
                max_value = matrix[i][2 * n - 1 - j];
            }
            if (matrix[2 * n - 1 - i][j] > max_value) {
                max_value = matrix[2 * n - 1 - i][j];
            }
            if (matrix[2 * n - 1 - i][2 * n - 1 - j] > max_value) {
                max_value = matrix[2 * n - 1 - i][2 * n - 1 - j];
            }
            upper_left += max_value;
        }
    }
